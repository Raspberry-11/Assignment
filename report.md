...
Storage was measured rather than asserted. Parquet is 4.58x smaller on disk and 1.8x faster on
a pass that aggregates all 450 values per frame, for a one-off conversion cost of 8.5 s that
pays for itself after about one and a half passes. Timing a row count instead would have
flattered Parquet badly, since it answers that from footer metadata without reading a range.
...
The aisle shift is real. A forest separates train frames from test frames at AUC 0.980 under
recording-wise cross-validation, against 0.689 for the same-aisle train-versus-val control,
...
Compared under an identical forest and protocol: the cutout reaches 0.287 val AP, the
fixed-±20-beam control 0.221, and the Arras 2007 descriptors 0.219. Since the control differs
...
The forest reaches 0.287 val AP, with peak F1 0.348 at precision 0.281 and recall 0.457.
...
The error analysis agrees. At the peak-F1 point there are 3,630 false positives and 1,685
misses, and both sit at a median range just under 5 m. It is not simply a far-field problem:
the detector fires too often in the same mid-range band where it is also missing people. About
a third of misses are past 6 m, which is the resolution limit, and 8% sit beyond ninety
degrees off-centre where the missing rate is highest. AP by band on val runs 0.454, 0.371,
0.170, 0.138 across 0-2, 2-4, 4-7 and 7-15 m, tracking the beams-on-target collapse.

On velocity: 13.7 ms per frame on CPU against a 78.7 ms budget at the measured rate, so 5.7x
headroom, at 285 MB steady state. The detector is stateless and single-frame, so that is also
end-to-end latency.
...
The hypothesis is mis-specified. There is no train to test drop. The detector scores 0.401 on
test against 0.287 on val, and higher in every range band. The question has no answer because
...
geometric, at AUC 0.980. The model genuinely leans on the background: 39% of the forest's
...
untargeted control wins: the mirror gains +0.034 test AP, the background arm +0.011, and both
